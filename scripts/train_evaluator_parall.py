...
def main():
    ...
    # ----- DUAL GPU ADAPTATION START ---
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    n_gpus = torch.cuda.device_count()
    print(f'Number of CUDA devices available: {n_gpus}')
    model = model.to(device)
    if n_gpus > 1:
        print("[INFO] Multiple GPUs detected. Wrapping model in DataParallel.")
        model = torch.nn.DataParallel(model)
    # ----- DUAL GPU ADAPTATION END ---
    ...
    train_dataloader = DataLoader(
        train, batch_size=batch_size, shuffle=True, sampler=None
    )
    ...
    for i, epoch in enumerate(range(num_epochs)):
        ...
        if hasattr(model, "module"):
            ...
            model_to_save = model
        model_to_save.save_pretrained(args.save_path + f"/ep{i}")
        tokenizer.save_pretrained(args.save_path + f"/ep{i}")
