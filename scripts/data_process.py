    ...
    with using_metadata(
        ...
        if dataset == "bio":
            with tracer.start_as_current_span(
                "bio_processing", openinference_span_kind="chain"
            ) as span:
                questions = []
                passages = []
                contents = []
            ...
            with tracer.start_as_current_span(
                "arc_processing", openinference_span_kind="chain"
            ) as span:
                questions = []
                passages = []
                choice_contents = []
            ...
            with tracer.start_as_current_span(
                "pubqa_processing", openinference_span_kind="chain"
            ) as span:
                questions = []
                passages = []
                contents = []
            ...
            with tracer.start_as_current_span(
                "popqa_processing", openinference_span_kind="chain"
            ) as span:
                questions = []
                passages = []
                contents = []
                for item in input_data:
                    ...
                    passages.append(" [sep] ".join(psgs))
                    contents += [
                        item["question"] + " [SEP] " + p + "\t" + l
                        for p, l in zip(psgs, label)
                    ]
        ...
        if pre:
            print("args.postprocess\t", args.postprocess)
            with tracer.start_as_current_span(
                "postprocess", openinference_span_kind="chain"
            ) as span:
                ...
                for i, (item, context, web_context, comb_context) in enumerate(
                    zip(pre_data, contexts, web_contexts, comb_contexts)
                ):
